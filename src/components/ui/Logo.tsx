...
const LOGO_SOURCES = getLocalImageSrcSet(LOGO_SRC);
  ...
  return (
    <div className={cn('flex shrink-0 items-center transition-[filter,transform] duration-500 ease-out', className)}>
      <picture>
        {LOGO_SOURCES ? (
          <>
            <source type="image/avif" srcSet={LOGO_SOURCES.avifSrcSet} />
            <source type="image/webp" srcSet={LOGO_SOURCES.webpSrcSet} />
          </>
        ) : null}
        <img
          src={LOGO_SRC}
